...
trait Validator {
    ...
    protected static function type(&$attribute, $param, &$errorMsg = null) {
        ...
        switch($param) {
            ...
            case 'float':
                ...
                $result |= $attribute === 0 || $attribute === '0';
     ...
     * @return boolean
     */
    protected static function name($attribute, $param, &$errorMsg = null) {
        $result = preg_match('/^[\w_-\s-()]+$/', $attribute);

        if(!$result && isset($errorMsg)) {
            $errorMsg = 'Only alphabetic chars, digits, dash, round brackets and underscore are enabled.';
        }
